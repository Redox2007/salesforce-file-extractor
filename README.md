...
LIMIT 50
```
**Files Linked to a Specific Record (ContentDocumentLink):**
```sql
...
WHERE LinkedEntityId = '0YA4N000000CfV1WAK'
AND ContentDocument.FileType != 'snote'
```
> **Note:** `ContentDocumentLink` queries must select `ContentDocumentId` and any `ContentDocument.*` fields you need (Title, FileExtension, FileType). Salesforce does not support subqueries on `ContentDocumentLink`, so use the `LinkedEntityId` filter directly in the `WHERE` clause.
